...
class TablesConstructor:
    ...
    def lattice_tables(self, connected_border_tolerance: float,
                       min_border_clearance: float, max_border_width: float):
        ...
        def remove_overlap(instances: list):
            ...
            fun = lambda a, b: a.bbox.contains(b.bbox) or b.bbox.contains(a.
                                                                          bbox)
        ...
        for strokes in grouped_strokes:
            ...
            table = TableStructure(
                strokes, **settings).parse(group_fills).to_table_block()
            if table:
    ...
    def stream_tables(self, min_border_clearance: float,
                      max_border_width: float, float_layout_tolerance: float,
        ...
        tables_lines = self._blocks.collect_stream_lines(
            table_fillings, float_layout_tolerance, line_separate_threshold)
        ...
        for table_lines in tables_lines:
            ...
            if not (explicit_shadings or explicit_strokes) and \
                TablesConstructor._is_simple_structure(table_lines):
                continue

            # parse stream borders based on lines in cell and explicit borders/shadings
            strokes = self._stream_strokes(table_lines, outer_borders,
                                           explicit_strokes, explicit_shadings)
    ...
    def _is_simple_structure(lines: Lines):
        ...
        * lines are aligned in each row -> simple paragraph in docx
        ...
        return len(lines.group_by_physical_rows()) == len(
            lines.group_by_rows())

    @staticmethod
    def _stream_strokes(lines: Lines, outer_borders: tuple,
                        explicit_strokes: Shapes, explicit_shadings: Shapes):
